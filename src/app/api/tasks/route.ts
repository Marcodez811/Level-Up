...
export async function GET(req: NextRequest) {
  try {
    ...
    await db.update(tasksTable)
        ...
        .where(and(eq(tasksTable.userId, user.id), eq(tasksTable.pause, false)));
    const tasks = await db.select().from(tasksTable).where(eq(tasksTable.userId, user.id)).orderBy(tasksTable.completed, desc(tasksTable.createdAt));
...
export async function PUT(req:NextRequest) {
  ...
  try {
    const { taskId, action } = data as UpdateTaskRequest;
    console.log(action);
    if (action !== "complete" && action !== "pause") return NextResponse.json({ error: "Invalid request" }, { status: 400 });
    const currentUser = await getCurrentUser();
    if (!currentUser) return NextResponse.json({ error: "Unauthorized" }, { status: 401 }); 
    // Check task ownership
    ...
    if(action==="complete") {
      ...
      console.log("[[[Pause]]]");
      await db.update(tasksTable).set({pause: sql`NOT ${tasksTable.pause}`}).where(eq(tasksTable.id, taskId));
    }
    return NextResponse.json(
        { status: 200 },
        )
